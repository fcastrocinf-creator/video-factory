...
- **Pendiente (próximas tandas):** (a) **receptor central del buzón** — DEBE **deduplicar por `id`** porque `sync.ts` reenvía 1 evento del borde por tanda (off-by-one inofensivo, `query` usa `desde` inclusivo); (b) trigger automático de sync (hook post-run o cron); (c) wizard que escriba las keys al `.env` interactivo; (d) dashboard multi-instalación.
