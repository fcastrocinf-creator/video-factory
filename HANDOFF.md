...
- **Receptor central + roles + dashboard ✅ (commit de esta tanda):** rol owner (`VF_ROLE=owner` → solo el owner ve `/admin`; `lib/auth.isOwner` + `(app)/layout.tsx` + `Sidebar.tsx` ownerOnly + `admin/page.tsx` redirect); receptor `POST /api/sync/ingest` (token `VF_SYNC_INGEST_KEY`) + `lib/kb/central-store.ts` (almacena por instalación en `storage/kb/central/`); trigger `syncPendingEventos()` al completar run (pipeline, opt-in/no-op); dashboard owner `GET /api/admin/central` + sección **"📊 Costo por usuario"** en `AdminPanel.tsx` (costo-eficiencia, ordenado por costo/video; costo = **ESTIMADO**, subcuenta clips). **Verificado:** typecheck VERDE + tests aislados (central-store ingest/read + centralCostStats parseo/orden) + guard 503 en vivo. `VF_ROLE=owner` agregado al `.env` del owner (**requiere reiniciar dev**).
